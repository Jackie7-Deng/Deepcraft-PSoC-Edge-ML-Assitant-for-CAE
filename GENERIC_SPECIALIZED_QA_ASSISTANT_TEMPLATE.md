# 专属问答 AI 助手通用训练模板（精简增强版）

> 目标：把一堆原始资料训练成一个有边界、会查证、会引用、可持续演进的专属问答助手。  
> 适用：Copilot CLI、Claude Code、ChatGPT 类 Agent，或任何 `Agent + 本地知识库 + 可选工具/MCP` 组合。
...
- `mcp_regression_suite.md` 或 `mcp_regression_suite.json`
...
- 轻量 RAG：本地 chunk index + search 脚本
- 更细的专题知识文档
...
- 如果直接在 VS Code / Cursor 里使用，且上下文、session、工具切换都已够用，阶段 4 可以暂缓。
- 阶段 4 更适合“要给更多同事复用”或“要做成独立支持工具”的场景。
...
如果你希望这个专属助手可以长期维护，而不是一次性产物，建议至少固定以下文件：

- `TODO.md`：当前轮唯一进度源
- `plan_gap_analysis.md`：对照模板的差距评估
- `issues_log.md`：先记录后修复的缺陷登记
- `knowledge_governance.md`：知识治理和回灌规则
- `mcp_regression_suite.md`：固定 external search 回归集
- `chunk_schema.md`：轻量 RAG 的最小契约
- `rag_regression.md`：轻量 RAG 的固定查询回归集
- `evidence_scoring.md` / `evidence_arbitrator.py`：候选证据仲裁层
- `mcp_observability.md`：external search 可观测字段约定

一句话：

> **知识库要能回答，工具链要能复测，问题要能审计。**
