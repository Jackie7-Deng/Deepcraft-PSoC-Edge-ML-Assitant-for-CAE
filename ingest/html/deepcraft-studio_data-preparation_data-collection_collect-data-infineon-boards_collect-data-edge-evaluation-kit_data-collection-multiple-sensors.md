---
source_url: "https://developer.imagimob.com/deepcraft-studio/data-preparation/data-collection/collect-data-infineon-boards/collect-data-edge-evaluation-kit/data-collection-multiple-sensors"
...
2. - Navigate to the **Node Explorer** window, expand \*\*Boards> PSoC 6 AI \*\* and drag and drop the **IMU** sensor onto the canvas. Click on the IMU sensor node and set the frequency, accel range, gyro range and mode in the Properties window. In this example, we want to collect data from both accelerometer and magnetometer, so select the mode option as combined. To know more, refer to [Setting up the IMU Sensor](https://developer.imagimob.com/deepcraft-studio/data-preparation/data-collection/collect-data-infineon-boards/collect-data-edge-evaluation-kit/data-collection-imu#setting-up-the-imu-sensor)
3. - Navigate to the **Node Explorer** window, expand **Boards> PSOC Edge E84 Evaluation Kit** and drag and drop the **Magnetometer** sensor onto the canvas. Click on the Magnetometer sensor node and set the frequency in the Properties window.
